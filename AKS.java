...
{
	public long PASO1_START;
	public long PASO2_START;
	public long PASO3_START;
	public long PASO5_START;

	public long PASO1_END;
	public long PASO2_END;
	public long PASO3_END;
	public long PASO5_END;
	...
	public boolean isPrimeTimed()
  {
	...
	public boolean isPrime() 
	{
		...
		// ####################################################################################### PASO 1
		// #######################################################################################
		PASO1_START = System.currentTimeMillis(); // Start timer
		...
		do
		{
			...
			if( comparison == 0 )
			{
				...
				factor = base;
				n_isprime = false;
				PASO1_END = System.currentTimeMillis();// End timer
				return n_isprime;
			}
		...
		// ###########################################################PASO2
		// #######################################################################################
		PASO2_START = System.currentTimeMillis(); // Start timer
		double log = this.log();
		...
		PASO2_END = System.currentTimeMillis(); // End timer
		// #######################################################################################
		// #################################---PASO3---###########################################
		// #######################################################################################
		// If 1 < gcd(a,n) < n for some a <= r, output COMPOSITE
		PASO3_START = System.currentTimeMillis(); // Start timer
		for( BigInteger i = BigInteger.valueOf(2); i.compareTo(r) <= 0; i = i.add(BigInteger.ONE) )
		{
			...
			if ( gcd.compareTo(BigInteger.ONE) > 0 && gcd.compareTo(n) < 0 )
			{
				factor = i;
				n_isprime = false;
				PASO3_END = System.currentTimeMillis(); // End timer
				return false;
			}
		}
		PASO3_END = System.currentTimeMillis(); // End timer
		...
		// sqrt(totient)log(n)
		PASO5_START = System.currentTimeMillis();
		...
		for( int i = 1; i <= limit; i++ )
		{
			...
			if( !outcome.equals(p) )
			{
				...
				n_isprime = false;
				PASO5_END = System.currentTimeMillis();
				return n_isprime;
			}
			else
